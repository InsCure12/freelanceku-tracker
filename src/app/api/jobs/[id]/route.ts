...
const ALLOWED_FIELDS = ["clientName", "projectName", "date", "categoryId", "amount", "currency", "duration", "deadline", "status", "description"];
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (sanitized.clientName !== undefined) {
    if (typeof sanitized.clientName !== "string" || (sanitized.clientName as string).trim().length === 0) {
  ...
  if (sanitized.amount !== undefined) {
    if (typeof sanitized.amount !== "number" || !isFinite(sanitized.amount as number) || (sanitized.amount as number) < 0) {
      return NextResponse.json({ error: "Amount must be a non-negative number" }, { status: 400 });
    }
  }
  if (sanitized.currency !== undefined && !VALID_CURRENCIES.includes(sanitized.currency as string)) {
    return NextResponse.json({ error: "Currency must be IDR or USD" }, { status: 400 });
  }
  if (sanitized.status !== undefined && !VALID_STATUSES.includes(sanitized.status as string)) {
    return NextResponse.json({ error: "Status must be done, pending, or ongoing" }, { status: 400 });
  }
  if (sanitized.date !== undefined && !/^\d{4}-\d{2}-\d{2}$/.test(sanitized.date as string)) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.delete(job).where(and(eq(job.id, id), eq(job.userId, session.user.id)));

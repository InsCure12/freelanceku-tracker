...
export async function POST(req: NextRequest) {
  ...
  if (!clientName || !date || amount === undefined) {
    return NextResponse.json(
      { error: "Missing required fields: clientName, date, amount" },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    clientName.length > MAX_STRING_LEN
  ) {
    return NextResponse.json({ error: "Invalid clientName" }, { status: 400 });
  }
  if (
    projectName &&
    (typeof projectName !== "string" || projectName.length > MAX_STRING_LEN)
  ) {
  ...
  if (paymentStatus && !VALID_PAYMENT_STATUSES.includes(paymentStatus)) {
    return NextResponse.json(
      { error: "Payment status must be paid or unpaid" },
      { status: 400 },
    );
  }
  if (
  ...
  const newJob = await db
    ...
    .values({
      ...
      dpAmount: paymentStatus === "dp" && dpAmount ? Number(dpAmount) : 0,
      dpDate: paymentStatus === "dp" && dpDate ? dpDate : null,

...
import { formatIDR, formatUSD, formatAmount, toIDR, toUSD } from "@/lib/currency";
...
export default function FreeDashboard({ userName, totalIncomeIDR, totalIncomeUSD, jobCount, recentJobs, categories, selectedMonth }: FreeDashboardProps) {
  ...
  const heroSub = cur === "IDR" ? `≈ $${formatUSD(totalIncomeUSD)}` : `≈ Rp ${formatIDR(totalIncomeIDR)}`;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
        ...
        <div className="lg:col-span-8 bg-gradient-to-br from-primary via-primary-container to-tertiary-container rounded-3xl p-6 lg:p-8 text-on-primary animate-fade-in-up relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between">
              <p className="text-[0.7rem] uppercase tracking-[0.15em] font-semibold text-on-primary/70">Total Income This Month</p>
            ...
            <div className="flex items-baseline gap-4 mt-2">
              <h2 className="text-3xl lg:text-5xl font-extrabold tabular-nums tracking-tight">
                {cur === "IDR" ? `Rp ${formatIDR(heroAmount)}` : `$${formatUSD(heroAmount)}`}
              </h2>
        ...
        <div className="lg:col-span-4 bg-surface-container-lowest rounded-2xl p-6 animate-fade-in-up delay-200">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider">Job Usage</h3>
          ...
          <div className="mt-4">
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-bold text-on-surface">{jobCount}</span>
              <span className="text-lg text-outline">/ 50</span>
            </div>
            <p className="text-xs text-outline mt-1">
              You have used {Math.round((jobCount / 50) * 100)}% of your monthly job entries allowed in the free tier.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
        ...
        <div className="lg:col-span-4 animate-slide-left delay-300">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-on-surface uppercase tracking-wide">Categories</h3>
          ...
          <div className="space-y-3">
            {categories.length === 0 ? (
              ...
              categories.map((cat) => (
                <div key={cat.id} className="bg-surface-container-lowest rounded-xl p-4 flex items-center gap-4 hover-lift cursor-default animate-list-item" style={{ animationDelay: `${0.4 + categories.indexOf(cat) * 0.08}s` }}>
        ...
        <div className="lg:col-span-8 animate-slide-right delay-300">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm lg:text-lg font-bold text-primary lg:text-on-surface uppercase tracking-wide">Recent Jobs</h3>
            <Link href="/dashboard/work-log" className="flex items-center gap-1 text-[0.65rem] lg:text-sm text-primary font-bold lg:font-medium uppercase lg:normal-case tracking-wider lg:tracking-normal hover:underline">
              See all
            </Link>
          </div>
          ...
          <div className="lg:hidden space-y-3">
            {filteredJobs.length === 0 ? (
              <div className="py-8 text-center text-sm text-outline">No jobs yet. Start by logging your first income!</div>
            ) : (
              filteredJobs.map((j) => {
            ...
            <table className="w-full">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-outline">
                  ...
                  <th className="text-left py-4 px-5 font-semibold">Client / Job</th>
              ...
              <tbody>
                {filteredJobs.length === 0 ? (
                  ...
                  filteredJobs.map((j, idx) => (
                    <tr key={j.id} className="hover:bg-surface-container-low/50 row-highlight animate-list-item" style={{ animationDelay: `${0.5 + idx * 0.06}s` }}>
      ...
      <div className="bg-surface-container-lowest rounded-2xl p-5 lg:p-6 flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-6 animate-fade-in-up delay-500">
        ...
        <div className="flex-1">
          <h3 className="text-base lg:text-lg font-bold text-on-surface">Scale your business with Architect Pro</h3>
          <p className="text-xs lg:text-sm text-on-surface-variant mt-1">
            Unlock unlimited jobs, automated invoicing, tax projections, and advanced financial analytics to take your freelancing to the next level.
          </p>
        </div>

...
function InvoiceStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${styles[status] || styles.draft}`}
    >
      {status}
    </span>
  );
...
export default function InvoicesClient({ invoices }: { invoices: Invoice[] }) {
  ...
  const paidTotal = paidInvoices.reduce(
    (s, inv) => s + (inv.paidAmount || inv.total),
    0,
  );
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      ...
      const newStatus =
        newPaidAmount >= payingInvoice.total
          ? "paid"
          : payingInvoice.status === "draft"
            ? "sent"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
        <div className="bg-gradient-to-r from-primary to-primary-container rounded-2xl p-5 lg:p-6 text-on-primary">
          ...
          <p className="text-3xl lg:text-4xl font-bold mt-2">
            Rp {formatIDR(totalReceivables)}
          </p>
          <p className="text-xs text-primary-fixed/60 mt-3">
        ...
        <div className="bg-surface-container-lowest rounded-2xl p-5 lg:p-6">
          ...
          <p className="text-3xl lg:text-4xl font-bold mt-2 text-on-surface">
            Rp {formatIDR(agingAmount)}
          </p>
          <p className="text-xs text-outline mt-1">
            Across {overdueInvoices.length} overdue accounts
          </p>
            ...
            paginated.map((inv) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      ...
                      style={{
                        background: `hsl(${inv.clientName.charCodeAt(0) * 10}, 60%, 90%)`,
                        color: `hsl(${inv.clientName.charCodeAt(0) * 10}, 60%, 30%)`,
                      }}
                    >
                      {inv.clientName
                        .split(" ")
                ...
                <div className="flex items-center justify-between mt-3">
                  <span
                    className={`text-sm font-bold ${inv.status === "overdue" ? "text-error" : "text-on-surface"}`}
                  >
                  ...
                  <div className="flex gap-1">
                    {inv.status !== "paid" && (
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-[#dcfce7] text-[#166534]"
                        title="Mark as Paid"
                      >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg hover:bg-surface-container-high text-outline ${downloading === inv.id ? "opacity-50" : ""}`}
                      title="Download PDF"
                    >
                      <ArrowDownToLine size={16} strokeWidth={1.75} />
                    </button>
        ...
        {invoices.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between px-4 lg:px-6 py-4 gap-3">
            <p className="text-xs text-outline">
              ...
              {Math.min(page * itemsPerPage, invoices.length)} of{" "}
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from(
                ...
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-xs font-semibold ${p === page ? "bg-primary text-on-primary" : "hover:bg-surface-container-high text-on-surface-variant"}`}
                >
                  {p}
                </button>
              ))}
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-on-surface">
                  Rp {formatIDR(payingInvoice.total)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-outline">Already Paid</span>
                <span className="font-semibold text-on-surface">
                  Rp {formatIDR(payingInvoice.paidAmount || 0)}
                </span>
              </div>
              <div className="flex justify-between text-sm border-t border-outline/20 pt-2">
                ...
                <span className="font-bold text-primary">
                  Rp{" "}
                  {formatIDR(
                    payingInvoice.total - (payingInvoice.paidAmount || 0),
                  )}
                </span>
              </div>
              <div>

...
export default function TaxPlannerClient({
  totalIncome,
  monthlyData,
  actualTaxAmount,
  totalInvoicedAmount,
}: TaxPlannerClientProps) {
  const estimatedTax =
    actualTaxAmount > 0 ? actualTaxAmount : totalIncome * 0.1;
  const smartSavings = totalIncome * 0.023;
  const projectedGross =
    totalInvoicedAmount > 0 ? totalInvoicedAmount : totalIncome;
  const netProfit = projectedGross - estimatedTax;
  ...
  const strategies = [
    {
      ...
      description:
        "Maxing out your SEP IRA contributions could reduce your taxable income based on current revenue.",
      meta: [
        "Verified Logic",
        `Potential: Rp ${formatIDR(totalIncome * 0.015)} Saved`,
      ],
    },
    {
      ...
      description:
        "New laptop and office hardware purchases qualify for immediate 100% deduction in this tax year.",
    ...
    },
    {
      ...
      description:
        "Updated floor plan measurements suggest you can increase your deduction by 4.2%.",
  ...
  const deadlines = [
    {
      month: "Oct",
      day: "15",
      title: "Q3 Estimated Payments",
      type: "IRS Federal",
    },
    {
      month: "Nov",
      day: "20",
      title: "VAT Reporting Period",
      type: "Regional Revenue",
    },
    {
      month: "Jan",
      day: "15",
      title: "Q4 Final Settlement",
      type: "Annual Closure",
    },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
        <div className="lg:col-span-8 bg-surface-container-lowest rounded-2xl p-5 lg:p-8">
          <p className="text-[10px] uppercase tracking-wider text-outline">
            Real-Time Estimated Tax
          </p>
          <p className="text-3xl lg:text-5xl font-bold text-on-surface mt-2">
            Rp {formatIDR(estimatedTax)}
          </p>
          <div className="flex items-center gap-4 mt-3">
            <span className="text-xs text-[#16a34a]">
              ↘ 12.5% decrease from last projection
            </span>
            <span className="text-xs text-outline">
              Next payment due: Oct 15
            </span>
          </div>
        ...
        <div className="lg:col-span-4 bg-gradient-to-br from-tertiary to-tertiary/80 rounded-2xl p-5 lg:p-6 text-on-tertiary">
          ...
          <p className="text-3xl lg:text-4xl font-bold mt-2">
            Rp {formatIDR(smartSavings)}
          </p>
          <p className="text-sm text-on-tertiary/70 mt-3">
            Our logic engine identified 4 new deductible expenses and 1 tax
            credit you qualify for this quarter.
          </p>
            ...
            {strategies.map((s, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase ${s.badgeColor}`}
                  >
                    {s.badge}
                  </span>
                </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {deadlines.map((d, i) => (
                <div
                  key={i}
                  className="bg-surface-container-lowest rounded-xl p-4 flex items-center gap-4 hover:bg-surface-container-low transition-colors cursor-pointer"
                >

...
export default async function TaxPlannerPage() {
  ...
  const yearJobs = jobs.filter((j) => j.date.startsWith(currentYear));
  const totalIncome = yearJobs.reduce(
    (s, j) => s + (j.currency === "IDR" ? j.amount : j.amount * EXCHANGE_RATE),
    0,
  );

  // Fetch invoices for actual tax data
  const invoices = await db.query.invoice.findMany({
    where: eq(invoice.userId, session.user.id),
  });
  const yearInvoices = invoices.filter((inv) =>
    inv.issueDate.startsWith(currentYear),
  );
  const paidInvoices = yearInvoices.filter((inv) => inv.status === "paid");
  const actualTaxAmount = paidInvoices.reduce(
    (s, inv) =>
      s +
      (inv.currency === "IDR"
        ? (inv.taxAmount ?? 0)
  ...
  const totalInvoicedAmount = paidInvoices.reduce(
    (s, inv) =>

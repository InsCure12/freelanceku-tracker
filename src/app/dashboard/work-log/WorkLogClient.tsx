...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${styles[status] || styles.pending}`}
    >
      {status}
    </span>
  );
...
function PaymentBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${s.bg}`}>
      {s.label}
    </span>
  );
}
...
function CategoryBadge({ name }: { name: string }) {
  ...
  const style =
    colors[name] || "bg-surface-container-high text-on-surface-variant";
  return (
...
export default function WorkLogClient({
  ...
  const avgRate =
    totalHours > 0
      ? jobs.reduce(
          (s, j) => s + (j.currency === "USD" ? j.amount : j.amount / 15745),
          0,
        ) / totalHours
      : 0;
    ...
    <div className="space-y-6">
      ...
      {plan === "pro" && (
        <div className="text-xs text-outline uppercase tracking-wider">
          Projects &gt;{" "}
          <span className="text-on-surface font-semibold">Work Log</span>
        </div>
      )}
        ...
        <div>
          ...
          {plan === "free" && (
            <p className="text-xs lg:text-sm text-on-surface-variant mt-1">
              Detailed history of your architectural and design commissions.
            </p>
          )}
        </div>
        <div className="flex gap-3">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {plan === "pro" ? "Log New Hours" : "New Entry"}
            </span>
          </Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
          <div className="bg-surface-container-lowest rounded-2xl p-5 lg:p-6">
            ...
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-3xl lg:text-4xl font-bold text-on-surface">
          ...
          <div className="bg-gradient-to-r from-tertiary to-tertiary/80 rounded-2xl p-5 lg:p-6 text-on-tertiary">
            ...
            <p className="text-xs text-on-tertiary/60 mt-2">
              Blended rate across IDR/USD contracts
            </p>
          ...
          <div className="bg-gradient-to-br from-primary to-primary-container rounded-2xl p-5 lg:p-6 text-on-primary">
            ...
            <p className="text-xs text-primary-fixed/60 mt-1">
              Multi-currency exchange rates auto-syncing every 60m.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 lg:gap-4">
        <div className="flex items-center gap-2 px-4 py-2.5 bg-surface-container-lowest rounded-xl">
          ...
          >
            <option value="all">
              {plan === "pro" ? "Last 30 Days" : "All Months"}
            </option>
            {Array.from(new Set(jobs.map((j) => j.date.slice(0, 7))))
              .sort()
              .reverse()
              .map((m) => (
            ...
            >
              ...
              <div className="flex flex-col items-end shrink-0 ml-3">
                <span className="text-sm font-bold text-primary">
                  Rp{" "}
                  {formatIDR(
                    j.currency === "IDR" ? j.amount : j.amount * 15745,
                  )}
                </span>
                <div className="flex gap-1 mt-1">
                  <StatusBadge status={j.status} />
                  <PaymentBadge status={j.paymentStatus} />
                </div>
              </div>
            </div>
          ))
          ...
          <thead>
            <tr className="text-[10px] uppercase tracking-wider text-outline">
              ...
              <th className="text-left py-4 px-6 font-semibold">
                {plan === "pro" ? "Project & Client" : "Client / Job"}
              </th>
              {plan === "pro" && (
              ...
              <th className="text-center py-4 px-6 font-semibold">Payment</th>
                  ...
                  {plan === "pro" && (
                    <td className="py-4 px-6 text-right">
                      <p className="text-sm font-bold text-on-surface">
                        $
                        {formatUSD(
                          j.currency === "USD" ? j.amount : j.amount / 15745,
                        )}
                      </p>
            ...
            <div className="flex items-center gap-2">
              <div className="w-24 h-1.5 bg-surface-container-high rounded-full">
                <div
                  ...
                  style={{ width: `${(jobCount / 50) * 100}%` }}
                />
              </div>
              <span className="text-[10px] text-outline">
                {Math.round((jobCount / 50) * 100)}% of Monthly Limit
              </span>
            </div>
          )}

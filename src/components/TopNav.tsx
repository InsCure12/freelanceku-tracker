...
import { BellRing, CircleHelp, Search, SlidersHorizontal, AlertTriangle, Clock, XCircle, CalendarClock, CirclePause } from "lucide-react";
    ...
    <div className="absolute right-0 top-full mt-2 w-80 bg-surface-container-lowest rounded-2xl shadow-xl border border-outline-variant/15 overflow-hidden z-50">
      ...
      <div className="max-h-96 overflow-y-auto">
        {notifications.length === 0 ? (
          <div className="px-4 py-8 text-center text-sm text-outline">All clear — no alerts</div>
        ) : (
          notifications.map((n, i) => {
            const icon =
              n.type === "deadline"
                ? n.daysLeft < 0 ? <XCircle size={16} /> : n.daysLeft <= 3 ? <AlertTriangle size={16} /> : <Clock size={16} />
                : n.type === "upcoming"
                  ? <CalendarClock size={16} />
                  : <CirclePause size={16} />;
            const iconStyle =
              n.type === "deadline"
                ? n.daysLeft <= 0 ? "bg-error/10 text-error" : n.daysLeft <= 3 ? "bg-error/10 text-error" : "bg-orange-500/10 text-orange-500"
                : n.type === "upcoming" ? "bg-primary/10 text-primary" : "bg-orange-500/10 text-orange-500";
            const labelColor =
              n.type === "deadline"
                ? n.daysLeft <= 3 ? "text-error" : "text-orange-500"
                : n.type === "upcoming" ? "text-primary" : "text-orange-500";
            const bgColor =
              n.type === "deadline" && n.daysLeft < 0 ? "bg-error/5" : n.type === "pending" ? "bg-orange-500/5" : "";
            const typeLabel =
              n.type === "deadline" ? "Deadline" : n.type === "upcoming" ? "Upcoming" : "Not started";

            return (
              <div key={`${n.type}-${n.id}-${i}`} className={`px-4 py-3 border-b border-outline-variant/5 last:border-b-0 ${bgColor}`}>
                <div className="flex items-start gap-3">
                  <div className={`mt-0.5 p-1.5 rounded-lg ${iconStyle}`}>{icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium text-on-surface truncate">{n.clientName}</p>
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold shrink-0 ${iconStyle}`}>{typeLabel}</span>
                    </div>
                    {n.projectName && <p className="text-xs text-outline truncate">{n.projectName}</p>}
                    <p className={`text-xs mt-1 font-medium ${labelColor}`}>
                      {n.label}
                      {n.type === "deadline" && n.deadline && (
                        <>{" · "}<span className="text-outline font-normal">{new Date(n.deadline).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span></>
                      )}
                      {n.type === "upcoming" && (
                        <>{" · "}<span className="text-outline font-normal">{new Date(n.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span></>
                      )}
                    </p>
                  </div>
                </div>
...
export default function TopNav({ plan, userName, notifications = [] }: TopNavProps) {
    ...
    <header className="sticky top-0 z-30 bg-surface-container-lowest/80 backdrop-blur-xl border-b border-outline-variant/10 transition-all duration-200">
      ...
      <div className="flex items-center justify-between px-6 py-4 lg:hidden">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-bold text-primary tracking-tight">The Fiscal Architect</span>
        </div>
      ...
      <div className="hidden lg:flex items-center justify-between px-6 py-3">
        ...
        <div className="relative flex-1 max-w-md">
          <Search size={18} strokeWidth={1.75} className="absolute left-3 top-1/2 -translate-y-1/2 text-outline" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder={plan === "free" ? "Search jobs or clients..." : "Search entries..."}
        ...
        <div className="flex items-center gap-3 ml-4">
          ...
          {plan === "pro" && (
            <Link href="/dashboard/settings" className="p-2 rounded-lg hover:bg-surface-container-high/50 text-on-surface-variant transition-colors">
              <SlidersHorizontal size={20} strokeWidth={1.75} />
            </Link>
          )}
          {/* Profile */}
          <div className="flex items-center gap-2 ml-2">
            <span className="text-sm text-on-surface-variant hidden sm:block">{userName || "User"}</span>
            {plan === "free" && (
